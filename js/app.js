...
const BUILD_TIME = String(import.meta.env.VITE_BUILD_TIME || 'desconocido');
...
function renderTabs(activa) {
  ...
  tabs.innerHTML = `${botones}<span class="version-badge" title="Build UTC: ${BUILD_TIME}">Versión: ${APP_VERSION}</span>`;
...
function renderVista(v) {
  ...
  if (v === 'excel') {
    ...
    document.getElementById('excel-btn').onclick = async () => {
      ...
      document.getElementById('excel-msg').textContent = `Importación completada: ${res.nuevos} pallets nuevos (${res.creadosPorCantidad || 0} generados desde columna D - pallets).`;
    };
  }
