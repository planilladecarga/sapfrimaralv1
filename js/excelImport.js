    ...
    fr.onload = () => {
      try {
        ...
        rows.forEach((r) => {
          ...
          const descripcionStock = normalizarTexto(r[6]);
          ...
          const descripcionAlt = normalizarTexto(r[1]);

          const usarFormatoStock = Boolean(contenedorStock && cantidadStock > 0);
          const destinoContenedor = usarFormatoStock ? contenedorStock : contenedorAlt;
          const cantidadFinal = usarFormatoStock ? cantidadStock : Math.max(1, cantidadAlt || 1);
          const kilosTotales = usarFormatoStock ? kilosStock : toNumber(r[5], toNumber(r[4], 0));
          const descripcionProductoFinal = usarFormatoStock ? descripcionStock : descripcionAlt;
          const lote = normalizarTexto(r[1]) || normalizarTexto(r[0]);

          if (!destinoContenedor || !descripcionProductoFinal || cantidadFinal <= 0) return;

          crearContenedor(destinoContenedor, destinoContenedor);

          const kilosPorPallet = cantidadFinal > 0 ? (kilosTotales / cantidadFinal) || 0 : 0;

          for (let i = 0; i < cantidadFinal; i += 1) {
            while (idsExistentes.has(nextId)) nextId += 1;

            crearPallet({
              id: nextId,
              cliente: clienteActual,
              producto: descripcionProductoFinal,
              lote,
              contenedor: destinoContenedor,
            ...
            nextId += 1;
            nuevos += 1;
            if (cantidadFinal > 1) creadosPorCantidad += 1;
          }
        });
